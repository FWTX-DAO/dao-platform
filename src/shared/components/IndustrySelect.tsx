    ...
    >
      <div className={`relative ${className ?? ""}`}>
        ...
        <ComboboxButton
          ...
          aria-label="Toggle industry list"
        >
          <ChevronsUpDown
            className={`h-5 w-5 ${isDark ? "text-dao-cool/40" : "text-gray-400"}`}
          />
        </ComboboxButton>
    ...
    <ComboboxOption value={item} className={optionClass}>
      {({ selected }) => (
        <>
          <span>
            <span
              className={`font-mono text-xs mr-2 ${isDark ? "text-dao-cool/50" : "text-gray-400"}`}
            >
              {item.code}
            </span>
            {item.label}
            <span
              className={`ml-2 text-xs ${isDark ? "text-dao-cool/40" : "text-gray-400"}`}
            >
              ({item.sector})
            </span>
          </span>
          {selected && (
            <Check
              className={`h-4 w-4 shrink-0 ${isDark ? "text-dao-gold" : "text-violet-600"}`}
            />
          )}
        </>
      )}
