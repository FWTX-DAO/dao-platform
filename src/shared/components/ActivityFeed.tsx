      ...
      <ul className="space-y-4">
        {feed.map((item: PlatformActivity) => {
          const meta = ACTIVITY_META[item.activityType] || {
            label: item.activityType || "performed an action",
            icon: ActivityIcon,
          };
          ...
          return (
            <li key={item.id} className="relative flex items-start gap-3 pl-2">
              <div
                className="relative z-10 flex h-8 w-8 items-center justify-center rounded-full bg-violet-100 text-violet-600 shrink-0"
                aria-hidden="true"
              >
      ...
      <ul className="space-y-4">
        {activities.map((item: Activity) => {
          ...
          return (
            <li key={item.id} className="relative flex items-start gap-3 pl-2">
              <div
                className="relative z-10 flex h-8 w-8 items-center justify-center rounded-full bg-gray-100 text-gray-600 shrink-0"
                aria-hidden="true"
              >
                <Icon className="h-4 w-4" />
              </div>
