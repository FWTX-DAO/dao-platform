...
export class SubscriptionsService {
  ...
  private getStripe(): Stripe {
    if (!this._stripe) {
      this._stripe = new Stripe(process.env.STRIPE_SECRET_KEY!, { apiVersion: '2026-02-25.clover' });
    }
    return this._stripe;
  }
  ...
  async updateFromWebhook(event: Stripe.Event) {
    switch (event.type) {
      case 'checkout.session.completed': {
        ...
        const stripeSubRef = typeof session.subscription === 'string'
          ? session.subscription
          : (session.subscription as Stripe.Subscription)?.id;

        // Idempotency: skip if subscription already recorded
        if (stripeSubRef) {
          const existing = await this.repository.findByStripeSubscriptionId(stripeSubRef);
          if (existing) break;
        }
        ...
        if (stripeSubRef) {
          const stripeSub = await this.getStripe().subscriptions.retrieve(stripeSubRef);
