...
function validateUsernameFormat(username: string) {
  ...
  if (!/^[a-zA-Z0-9][a-zA-Z0-9_-]*$/.test(username))
    return {
      valid: false,
      error:
        "Must start with a letter or number. Letters, numbers, underscores, hyphens only",
    };
...
export function OnboardingForm() {
  ...
  const handlePostOnboarding = async () => {
    if (selectedTierId) {
      ...
      try {
        ...
        console.error(
          "[onboarding] Checkout failed:",
          data.error || "No checkout URL returned",
        );
        setGlobalError(
          data.error || "Could not start checkout. Redirecting to dashboard.",
        );
      } catch (err) {
        console.error("[onboarding] Checkout request failed:", err);
        setGlobalError(
          "Could not connect to payment service. Redirecting to dashboard.",
        );
      }
    }
  ...
  const isEmailFromPrivy =
    !!formData.email &&
    (formData.email === user?.email?.address ||
      formData.email === (user?.google as any)?.email);
          ...
          <div className="flex-1 flex items-center justify-center px-4 sm:px-6 py-8 md:py-12">
            ...
            >
              <div className="bg-dao-dark/80 backdrop-blur-xs border border-dao-border/60 rounded-lg p-6 sm:p-8 md:p-10">
                ...
                {step === 0 && (
                  <div className="space-y-5">
                    ...
                    <div className="flex items-start gap-3 pt-1">
                      ...
                      <label
                        htmlFor="terms"
                        className="text-sm text-dao-cool/70"
                      >
                        I accept the{" "}
                        <a
                ...
                {step === 1 && (
                  <div className="space-y-5">
                    <p className="text-sm text-dao-cool/50 mb-1">
                      All fields are optional. You can update these later in
                      your profile.
                    </p>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                    ...
                    <div>
                      ...
                      <IndustrySelect
                        ...
                        onChange={(code) => updateField("industry", code)}
                        variant="dark"
                      />
                      <p className="mt-1 text-[11px] text-dao-cool/40">
                        Type to search or browse by name, sector, or code
                      </p>
                    </div>
                    <div>
                ...
                {step === 2 && (
                  <div className="space-y-5">
                    <p className="text-sm text-dao-cool/50 mb-1">
                      Help us connect you with the right initiatives.
                    </p>
                    <div>
                      <label htmlFor="civicInterests" className={labelBase}>
                        Civic Interests
                      </label>
                      <textarea
                        id="civicInterests"
                        ...
                        onChange={(e) =>
                          updateField("civicInterests", e.target.value)
                        }
                        placeholder="e.g., urban planning, education, public safety"
                        rows={3}
                        className={`${inputBase} resize-none`}
                      />
                      <p className="text-[11px] text-dao-cool/40 mt-1">
                    ...
                    <div className="pt-4 border-t border-dao-border/30">
                      ...
                      <p className="text-[11px] text-dao-cool/40 mb-3">
                        Connect and verify your wallet with a signature. No gas
                        fees. You can also do this later in Settings.
                      </p>
                      ...
                      {walletVerified && walletAddress ? (
                        <div className="flex items-center gap-2 px-3 py-2.5 bg-green-500/10 border border-green-500/20 rounded-sm">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs font-mono text-green-400/70 truncate">
                              {walletAddress}
                            </p>
                          </div>
                        </div>
                        ...
                        <div className="space-y-2.5">
                          <div className="flex items-center gap-2 px-3 py-2.5 bg-dao-surface border border-dao-border rounded-sm">
                            <span className="text-xs font-mono text-dao-cool/60 truncate">
                              {privyEthWallet.address}
                            </span>
                          </div>
                          <button
                ...
                {globalError && (
                  <div
                    role="alert"
                    className="mt-5 p-3 bg-red-500/10 border border-red-500/20 rounded-sm"
                  >
