...
export function PlanSelector({ selectedTierId, onSelect }: PlanSelectorProps) {
  ...
  if (isLoading) {
    return (
      <div className="py-8 text-center text-dao-cool/40 text-sm">
        Loading plans{"…"}
      </div>
    );
  }
  ...
  const monthlyTier = tiers.find(
    (t) => t.billingInterval === "month" && t.priceCents > 0,
  );
  const annualTier = tiers.find(
    (t) => t.billingInterval === "year" && t.priceCents > 0,
  );
  ...
  const isPaidSelected =
    selectedTierId && activeTier && selectedTierId === activeTier.id;
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-center">
        <div className="inline-flex bg-dao-surface border border-dao-border rounded-lg p-0.5">
          <button
            ...
            className={`px-5 py-2 text-sm font-medium rounded-md transition-colors focus-visible:ring-2 focus-visible:ring-dao-gold focus-visible:outline-hidden ${
              interval === "month"
      ...
      {activeTier && (
        <button
          ...
          className={`w-full text-left p-6 rounded-lg border transition-colors duration-200 focus-visible:ring-2 focus-visible:ring-dao-gold focus-visible:outline-hidden ${
            isPaidSelected
        ...
        >
          <div className="flex items-center justify-between mb-3">
            ...
            {isPaidSelected && (
              <svg
                className="h-5 w-5 text-dao-gold"
                fill="currentColor"
                viewBox="0 0 20 20"
              >
                <path
          ...
          <div className="mb-4">
            ...
            {interval === "year" && (
              <span className="ml-2 text-[11px] text-dao-gold/70 bg-dao-gold/10 px-2 py-0.5 rounded-sm">
                2 months free
              </span>
            )}
          </div>
          <ul className="space-y-2">
            {features.map((f, i) => (
              <li
                key={i}
                className="flex items-center gap-2 text-sm text-dao-cool/70"
              >
                <svg
                  className="h-4 w-4 text-dao-gold/60 shrink-0"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                  strokeWidth={2}
      ...
      <button
        ...
        className={`w-full text-center py-3 text-sm transition-colors focus-visible:ring-2 focus-visible:ring-dao-gold focus-visible:outline-hidden ${
          selectedTierId === null
      ...
      >
        {selectedTierId === null ? (
          <span className="flex items-center justify-center gap-2">
            <svg
              className="h-4 w-4 text-dao-gold"
              fill="currentColor"
              viewBox="0 0 20 20"
            >
              <path
