  ...
  const getActivityLevel = useCallback((stats: MembershipData["stats"]) => {
    ...
    if (total === 0)
      return {
        ...
        message: "Start contributing to earn points!",
      };
    if (total < 5)
      return {
        level: "Active",
    ...
    if (total < 10)
      return {
        ...
        message: "You're making an impact!",
      };
    return {
      level: "Champion",
      badge: "bg-violet-100 text-violet-700",
      message: "Outstanding contribution!",
    };
  }, []);
      ...
      {isError ? (
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <Link
              href="/members"
              className="bg-white shadow-xs hover:shadow-md rounded-lg p-6 transition-shadow cursor-pointer border border-gray-100 hover:border-violet-200 focus-visible:ring-2 focus-visible:ring-dao-gold focus-visible:outline-hidden"
            >
              ...
              <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between mb-6 gap-3">
                <div>
                  ...
                  <p className="mt-1 text-sm font-medium flex items-center gap-2">
                    <span
                      className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${activityLevel?.badge}`}
                    >
                      {activityLevel?.level}
                    </span>
                ...
                <Link
                  href="/settings"
                  className="text-sm text-violet-600 hover:text-violet-700 font-medium min-h-[44px] inline-flex items-center"
                >
                  Settings &rarr;
                </Link>
              </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-6">
                <dl>
                  ...
                  <dd className="text-xs text-gray-500 mt-0.5">
                    {calculateTenure(membershipData.user.createdAt)}
                  </dd>
                </dl>
                <dl>
                  <dt className="text-sm font-medium text-gray-600">
                  ...
                  <dd className="text-lg font-bold text-gray-900 mt-1 capitalize">
                ...
                <dl>
                  ...
                  <dd className="text-lg font-bold text-gray-900 mt-1 tabular-nums">
                    {membershipData.membership.votingPower}
                  </dd>
                </dl>
                <dl>
                  <dt className="text-sm font-medium text-gray-600">
              ...
              <div className="pt-6 border-t border-gray-200">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 gap-1">
                  ...
                  <p className="text-xs text-gray-500">
                    Contribute more to earn points &amp; increase voting power
                  </p>
                </div>
          ...
          {/* My Active Projects */}
          {dashboardStats?.userActiveProjects &&
            dashboardStats.userActiveProjects.length > 0 && (
              <Card>
                <CardHeader
                ...
                <CardContent>
                            ...
                            <div className="flex items-start justify-between">
                              <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-2">
                                  ...
                                  <span
                                    className={`text-xs px-2 py-1 rounded-full font-medium shrink-0 ${project.user_role === "creator" ? "bg-violet-100 text-violet-700" : "bg-blue-100 text-blue-700"}`}
                                  >
                                    {project.user_role}
                                  </span>
                                </div>
                                <p className="text-xs text-gray-500 mt-1">
                                  ...
                                  &middot; {project.collaborators} collaborator
                  ...
                  {dashboardStats.userActiveProjects.length > 5 && (
                    <p className="text-xs text-gray-500 mt-3 text-center">
                      and {dashboardStats.userActiveProjects.length - 5}{" "}
                      more&hellip;
                    </p>
                  )}
                </CardContent>
              ...
              <CardContent>
                {dashboardStats?.activeProjects.length === 0 ? (
                  <EmptyState
                    ...
                    action={
                      <UpgradeCTA
                        allowed={can.createProject}
                        feature="create projects"
                      >
                        <Link
                          href="/innovation-lab"
                          className="inline-flex items-center px-4 py-2.5 bg-violet-600 text-white rounded-md hover:bg-violet-700 font-medium text-sm transition-colors min-h-[44px]"
                        >
                          <Plus className="h-4 w-4 mr-2" />
                          Submit Project
                        </Link>
                  ...
                  <ul className="space-y-3">
                      ...
                      .map((project: any) => (
                        <li key={project.id}>
                          ...
                          >
                            ...
                            <p className="text-xs text-gray-500 mt-1">
                              by {project.creator_name || "Anonymous"} &middot;{" "}
                    ...
                    {(dashboardStats?.activeProjects.length ?? 0) > 5 && (
                      <p className="text-xs text-gray-500 text-center pt-2">
                        and {(dashboardStats?.activeProjects.length ?? 0) - 5}{" "}
                        more&hellip;
                      </p>
                    )}
                  </ul>
                      ...
                      <li key={post.id}>
                        ...
                        >
                          ...
                          <p className="text-xs text-gray-500 mt-1">
                            ...
                            {post.reply_count} replies &middot; {post.upvotes}{" "}
                            votes
                          </p>
                        </Link>
                      </li>
            ...
            <Card>
              ...
              <CardContent>
                {dashboardStats?.innovationAssetsRanking.length === 0 ? (
                  ...
                  <ul className="space-y-3">
                    {dashboardStats?.innovationAssetsRanking.map(
                      (bounty: any) => (
                        <li key={bounty.id}>
                          <Link
                          ...
                          >
                            <div className="flex items-start justify-between">
                              <div className="flex-1 min-w-0">
                                ...
                                <p className="text-xs text-gray-500 mt-1">
                                  {bounty.category} &middot;{" "}
                                  {bounty.proposalCount} proposal
