...
export async function POST(request: Request) {
  try {
    ...
    const ensureStripeCustomer = async (): Promise<string> => {
      if (stripeCustomerId) {
        try {
          ...
          console.warn(
            "[checkout] Stored customer invalid, creating new one. Old:",
            stripeCustomerId,
          );
        }
      }
    ...
    console.log(
      "[checkout] Creating checkout session for customer:",
      stripeCustomerId,
      "price:",
      priceId,
    );
