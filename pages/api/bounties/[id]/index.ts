...
async function isUserAdmin(userId: string): Promise<boolean> {
  ...
  const isScreener =
    Array.isArray(member.specialRoles) &&
    member.specialRoles.includes("screener");
...
async function handler(req: AuthenticatedRequest, res: NextApiResponse) {
  ...
  if (req.method === "GET") {
    ...
    const [proposalsCountResult, comments, userProposalResult, _viewCountUpdate] =
      await Promise.all([
        // Get proposals count
        db
    ...
    const userProposal =
      userProposalResult.length > 0 ? userProposalResult[0] : null;
    ...
    const sanitizedBounty = {
      ...
      organizationContact:
        isSubmitter || isAdmin || bounty.status === "published"
          ? bounty.organizationContact
          : null,
      // Show screening notes only to submitter or verified admin
      screeningNotes:
        isSubmitter || isAdmin ? bounty.screeningNotes : null,
